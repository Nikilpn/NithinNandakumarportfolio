...
- Prefers Three.js 3D background animations to span/flow across adjacent sections (e.g. About → Skills) rather than being confined to a single section — uses shared parent wrapper with absolute-positioned canvases so the animation crosses section boundaries seamlessly. Confidence: 0.1
- Prefers 3D background animations to remain contained within their own section component (e.g. inside About.jsx), not extracted to a shared parent wrapper — explicitly rejected the cross-section spanning approach as not what was intended. Confidence: 0.3
